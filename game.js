...
document.addEventListener('DOMContentLoaded', () => {
    ...
    load_assets([{
        ...
        'var': warriorImage, url: '/static/warrior.png'
    }, {
        'var': mageImage, url: '/static/mage.png'
    }],
        drawMain)
}, false);
...
function Boundary() {
    ...
    for (let line of boundaryLine) {
        if (line[0] === line[2]) {
            ...
            let minX1 = line[0] - halfLineRange;
            let maxX1 = line[0] + halfLineRange;
            let minY1 = Math.min(line[1], line[3]);
            let maxY1 = Math.max(line[1], line[3]);


            if (playerRight >= minX1 &&
                playerRight < maxX1 &&
                playerBottom > minY1 &&
                playerTop < maxY1) {
                player.x = minX1 - player.xSize / 2;
            }
            if (playerLeft > minX1 &&
                playerLeft < maxX1 &&
                playerBottom > minY1 &&
                playerTop < maxY1) {
                player.x = maxX1 + player.xSize / 2;
            }
        }
        if (line[1] === line[3]) {

            let minY2 = line[1] - halfLineRange;
            let maxY2 = line[1] + halfLineRange;
            let minX2 = Math.min(line[0], line[2]);
            let maxX2 = Math.max(line[0], line[2]);

            if (playerBottom > minY2 &&
                playerBottom < maxY2 &&
                playerRight > minX2 &&
                playerLeft < maxX2) {
                player.y = minY2 - player.ySize / 2;
            }
            if (playerTop >= minY2 &&
                playerTop < maxY2 &&
                playerRight > minX2 &&
                playerLeft < maxX2) {
                player.y = maxY2 + player.ySize / 2;
            }
